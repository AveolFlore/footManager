    ...
    <style>
        body {
            ...
            background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.5)), 
                        url('/assets/images/team.jpg') no-repeat center center fixed;
...
<body class="text-white min-h-screen font-sans antialiased">
            ...
            <div class="max-w-7xl mx-auto space-y-6">
                ...
                <div class="glass-panel p-6 rounded-2xl shadow-lg flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        ...
                        <p class="text-sm text-slate-200 mt-1">Configuration des effectifs et des catégories du Blue Lock</p>
                    </div>
                ...
                <?php if (isset($_GET['msg'])): ?>
                    <div id="success-notification" class="px-5 py-4 bg-white/20 border border-emerald-400 text-white rounded-xl shadow-lg backdrop-blur-sm flex items-center">
                ...
                <div class="w-full md:max-w-md">
                    <div class="relative glass-panel rounded-xl">
                        <i class="fas fa-search absolute left-4 top-3.5 text-slate-200"></i>
                        <input type="text" id="searchInput" placeholder="Rechercher une équipe..."
                            class="w-full pl-12 pr-4 py-3 bg-transparent border-none focus:ring-0 outline-none text-sm placeholder-slate-300 text-white">
                    </div>
                </div>
                ...
                <div class="glass-panel rounded-2xl shadow-lg overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-left">
                            <thead class="bg-black/20 text-slate-200 text-xs uppercase tracking-wider">
                                <tr>
                                    <th class="p-5">Nom</th>
                            ...
                            <tbody id="equipesTableBody" class="divide-y divide-white/10 text-white">
                                <?php
                                if (!empty($equipes)):
                                    foreach ($equipes as $e): ?>
                                        <tr class="hover:bg-white/10 transition-colors">
                                            <td class="p-5 font-semibold"><?= htmlspecialchars($e['nom']) ?></td>
                                            <td class="p-5">
                                            ...
                                            <td class="p-5 text-center">
                                                <a href="/admin-team-show?id=<?= $e['id'] ?>" class="text-slate-200 hover:text-blue-400 mx-1"><i class="fas fa-eye"></i></a>
                                                <a href="/admin-teamedit?id=<?= $e['id'] ?>" class="text-slate-200 hover:text-amber-400 mx-1"><i class="fas fa-edit"></i></a>
                                                <button onclick="openDeleteModal(<?= $e['id'] ?>)" class="text-slate-200 hover:text-rose-400 mx-1"><i class="fas fa-trash"></i></button>
                                            </td>
                                        </tr>
                                    <?php endforeach;
                                else: ?>
                                    <tr>
                                        <td colspan="5" class="p-10 text-center text-slate-300">Aucune équipe trouvée.</td>
                                    </tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
    ...
    <div id="deleteModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 hidden p-4">
        <div class="glass-panel p-8 rounded-2xl max-w-sm w-full shadow-2xl text-center">
            <div class="w-16 h-16 bg-rose-900/50 text-rose-400 rounded-full flex items-center justify-center mx-auto mb-4">
                <i class="fas fa-trash-alt text-xl"></i>
            </div>
            <h3 class="text-lg font-bold text-white mb-2">Supprimer l'équipe ?</h3>
            <p class="text-sm text-slate-200 mb-6">Cette action est irréversible.</p>
            <div class="flex gap-3">
                <button onclick="closeDeleteModal()" class="flex-1 py-2.5 bg-white/10 border border-white/20 text-white rounded-xl hover:bg-white/20">Annuler</button>
