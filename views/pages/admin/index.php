...
<head>
    ...
    <style>
        body {
            background: url('/assets/images/welcome.jpg') no-repeat center center fixed;
        ...
        <main class="p-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    ...
                    <p class="text-slate-600">Gestion des membres et du club</p>
                </div>
            ...
            <div class="bg-white-glass rounded-2xl shadow-lg p-6 mb-8">
                <h2 class="text-xl font-bold mb-6 flex items-center gap-2">
                    <i class="fas fa-user-clock text-amber-500"></i> Demandes en attente (<?= count($allDemandes) ?>)
                </h2>
                <div class="space-y-4">
                    <?php if (empty($demandes)): ?>
                        <p class="text-slate-500 italic">Aucune demande en attente.</p>
                    <?php else: ?>
                        <?php foreach ($demandes as $d): ?>
                            <div class="border border-slate-200 rounded-xl p-4 flex justify-between items-center bg-white/50">
                                <div>
                                    <p class="font-bold"><?= htmlspecialchars($d['nom'] . ' ' . $d['prenom']) ?></p>
                                    <p class="text-sm text-slate-500"><?= htmlspecialchars($d['email']) ?></p>
                                </div>
                ...
                <div class="mt-6 flex gap-2">
                    <?php for($i=1; $i<=$nbPagesDemandes; $i++): ?>
                        <a href="?page_d=<?= $i ?>&page_u=<?= $pageUsers ?>" class="px-3 py-1 border rounded <?= $pageDemandes == $i ? 'bg-blue-600 text-white' : 'bg-white hover:bg-slate-100' ?>"><?= $i ?></a>
                    <?php endfor; ?>
                </div>
            </div>

            <div class="bg-white-glass rounded-2xl shadow-lg overflow-hidden">
                <table class="w-full text-left">
                    <thead class="bg-slate-200/50 text-slate-700">
                        <tr>
                            <th class="p-4">Nom</th>
                    ...
                    <tbody>
                        <?php foreach ($users as $user): ?>
                            <tr class="border-t border-slate-200">
                                ...
                                <td class="p-4">
                                    <span class="px-2 py-1 rounded-full text-xs font-bold <?= $user['statut'] === 'actif' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' ?>">
                                        <?= htmlspecialchars($user['statut']) ?>
                                    </span>
                                </td>
                                <td class="p-4 text-right">
                                    <a href="/admin-edituser?id=<?= $user['id'] ?>" class="text-blue-600 hover:text-blue-800 mr-4"><i class="fas fa-edit"></i></a>
                                    <a href="/admin-deleteuser?id=<?= $user['id'] ?>" class="text-red-600 hover:text-red-800" onclick="return confirm('Confirmer la suppression ?')"><i class="fas fa-trash"></i></a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                ...
                <div class="p-4 border-t bg-slate-50/50">
                    <?php for($i=1; $i<=$nbPagesUsers; $i++): ?>
                        <a href="?page_d=<?= $pageDemandes ?>&page_u=<?= $i ?>" class="px-3 py-1 border rounded <?= $pageUsers == $i ? 'bg-blue-600 text-white' : 'bg-white hover:bg-slate-100' ?>"><?= $i ?></a>
                    <?php endfor; ?>
                </div>
            </div>
        </main>
